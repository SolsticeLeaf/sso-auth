...
const connectRedis = async () => {
    if (isConnected || await checkRedisHealth()) { return; }
    try {
...
const setValue = async (key: string, value: string): Promise<void> => {
    if (await checkRedisHealth()) { await redisClient.set(key, value); }
};

const getValue = async (key: string): Promise<string | null> => {
    if (await checkRedisHealth()) { return redisClient.get(key); }
    return null;
};
